...
// فئات المتجر: كل فئة تمنح رصيد حمام عادي + حمام من كل نوع خاص (بعيد/محلي/ذهبي)،
// وتفتح إطار ملف شخصي أجمل. كل فئة = ضعف الفئة السابقة + 20% زيادة، والسعر من 1$ إلى 5$.
const STORE_TIERS = [
  { tier: 1, price: 1, normalQty: 50,   specialQty: 5 },
  { tier: 2, price: 2, normalQty: 110,  specialQty: 11 },
  { tier: 3, price: 3, normalQty: 242,  specialQty: 24 },
  { tier: 4, price: 4, normalQty: 532,  specialQty: 53 },
  { tier: 5, price: 5, normalQty: 1171, specialQty: 117 }
];
const TIER_NAMES = ['', 'الفئة الأولى 🥉', 'الفئة الثانية 🥈', 'الفئة الثالثة 🥇', 'الفئة الرابعة 💎', 'الفئة الخامسة 👑'];
const DEFAULT_PRODUCTS = STORE_TIERS.map(t => ({
  id: 'tier_' + t.tier,
  name: `${TIER_NAMES[t.tier]} — ${t.normalQty} حمامة عادية + ${t.specialQty} من كل نوع خاص`,
  isTier: true,
  tier: t.tier,
  normalQty: t.normalQty,
  specialQty: t.specialQty,
  price: t.price,
  emoji: ['', '🥉', '🥈', '🥇', '💎', '👑'][t.tier]
}));
db.defaults({
...
// ترقية آمنة: لو القاعدة قديمة وفيها منتجات النظام السابق، نستبدلها بفئات المتجر الجديدة
const existingProducts = db.get('config.products').value() || [];
if (!existingProducts.length || !existingProducts[0].isTier) {
  db.set('config.products', DEFAULT_PRODUCTS).write();
}

const PIGEON_TYPES = ['عادية','سريعة','بعيدة المدى','مجهولة','ذهبية','مفاجأة','نادرة','محلية'];
const DAILY_SPECIAL_TYPES = ['بعيدة المدى', 'محلية', 'ذهبية'];
...
function checkDailyReset(user) {
  ...
  if (Date.now() - user.lastReset >= DAY_MS) {
    user.balance = (user.isPremium ? 15 : 10) + (user.balance || 0);
    user.lastReset = Date.now();
    user.inventory = user.inventory || {};
    DAILY_SPECIAL_TYPES.forEach(t => { user.inventory[t] = (user.inventory[t] || 0) + 1; });
    db.get('users').find({ id: user.id }).assign(user).write();
  }
  return user;
}
...
function publicUser(user) {
  const { id, name, email, avatar, bio, hobbies, reputation, balance, lastReset, inventory, filters, hintsEnabled, homeRegion, isAdmin, isPremium, premiumUntil, banned, createdAt, claimedLaunchPromo, storeFrameTier, myLang } = user;
  return { id, name, email, avatar, bio, hobbies, reputation, balance, lastReset, inventory, filters, hintsEnabled, homeRegion, isAdmin: !!isAdmin, isPremium: !!isPremium, premiumUntil, banned: !!banned, createdAt, claimedLaunchPromo: !!claimedLaunchPromo, storeFrameTier: storeFrameTier||0, myLang: myLang||'ar' };
}
...
app.patch('/api/me', authMiddleware, (req, res) => {
  const { avatar, hintsEnabled, filters, bio, hobbies, homeRegion, myLang } = req.body;
  const updates = {};
  ...
  if (typeof myLang === 'string') updates.myLang = myLang;
...
app.post('/api/chats/:id/messages', authMiddleware, (req, res) => {
  ...
  const { mediaType, dataUrl } = req.body; // mediaType: 'image' | 'video' (اختياري)
  if (!text && !dataUrl) return res.status(400).json({ error: 'الرسالة فارغة' });
  const chatRef = db.get('chats').find(c => c.id === req.params.id && c.participants.includes(req.user.id));
  const chat = chatRef.value();
  if (!chat) return res.status(404).json({ error: 'المحادثة غير موجودة' });
  if (chat.status !== 'active') return res.status(400).json({ error: 'المحادثة غير مفعّلة بعد' });

  const message = { from: req.user.id, text: text || (mediaType === 'video' ? '📹 فيديو' : '📷 صورة'), at: Date.now() };
  if (dataUrl && mediaType) { message.mediaType = mediaType; message.dataUrl = dataUrl; }
...
// ============ ترجمة فورية حقيقية (عبر MyMemory — مجانية بدون مفتاح API) ============
app.post('/api/translate', authMiddleware, async (req, res) => {
  const text = (req.body.text || '').trim();
  const to = (req.body.to || 'ar').trim();
  if (!text) return res.status(400).json({ error: 'لا يوجد نص للترجمة' });
  try {
    ...
    res.status(200).json({ translated: text, failed: true }); // نرجع النص الأصلي بهدوء عند فشل خدمة الترجمة الخارجية
  }
});
...
app.post('/api/purchase', authMiddleware, (req, res) => {
  ...
  const user = req.user;
  if (product.isTier) {
    user.balance += product.normalQty || 0;
    user.inventory = user.inventory || {};
    DAILY_SPECIAL_TYPES.forEach(t => { user.inventory[t] = (user.inventory[t] || 0) + (product.specialQty || 0); });
    if ((product.tier || 0) > (user.storeFrameTier || 0)) user.storeFrameTier = product.tier;
  } else if (product.grantType === 'premium') {
    user.isPremium = true;
    const base = (user.premiumUntil && user.premiumUntil > Date.now()) ? user.premiumUntil : Date.now();
